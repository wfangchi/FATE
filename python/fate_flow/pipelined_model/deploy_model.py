...
def deploy(config_data: dict):
    ...
    try:
        ...
        deploy_model = PipelinedModel(
            model_id=party_model_id, model_version=child_model_version
        )
        ...
        inference_dsl = parser.get_predict_dsl(
            ...
            setting_conf_prefix=os.path.join(
        ...
        if model_utils.compare_version(pipeline.fate_version, "1.5.0") == "gt":
            pipeline.parent_info = json_dumps(
                {"parent_model_id": model_id, "parent_model_version": model_version},
                byte=True,
            )
        ...
        if model_utils.compare_version(model_info["f_fate_version"], "1.5.0") == "eq":
            model_info["roles"] = model_info.get("f_train_runtime_conf", {}).get(
                "role", {}
            )
        ...
        return (
            100,
            f"deploy model of role {local_role} {local_party_id} failed, details: {str(e)}",
        )
    else:

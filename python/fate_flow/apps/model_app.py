...
from fate_flow.pipelined_model import (
    migrate_model,
    pipelined_model,
    publish_model,
    deploy_model,
...
def load_model():
    ...
    if request_config.get("job_id", None):
        ...
        if res_data:
            ...
            runtime_conf = (
                model_info.get("f_runtime_conf", {})
                if model_info.get("f_runtime_conf", {})
                else model_info.get("f_train_runtime_conf", {})
            )
            ...
            request_config["job_parameters"] = (
                job_parameters
                if job_parameters
                else model_info.get("f_train_runtime_conf", {}).get("job_parameters")
            )
            roles = runtime_conf.get("role")
            request_config["role"] = (
                roles
                if roles
                else model_info.get("f_train_runtime_conf", {}).get("role")
            )
...
def migrate_model_process():
    ...
    try:
        if compare_roles(
            request_config.get("migrate_role"), request_config.get("role")
        ):
            return get_json_result(
                retcode=100,
        ...
        if not retcode:
            with DB.connection_context():
                model = MLModel.get_or_none(
                    ...
                    MLModel.f_model_id
                    == request_data.get("job_parameters").get("model_id"),
...
def operate_model(model_operation):
    ...
    if model_operation not in [
        ModelOperation.STORE,
        ModelOperation.RESTORE,
        ModelOperation.EXPORT,
        ModelOperation.IMPORT,
    ]:
        raise Exception(
    ...
    if model_operation in [ModelOperation.EXPORT, ModelOperation.IMPORT]:
        if model_operation == ModelOperation.IMPORT:
            try:
                ...
                pipeline = model.read_component_model("pipeline", "pipeline")[
                    "Pipeline"
                ]
                ...
                try:
                    ...
                    if not db_model:
                        ...
                        if (
                            model_utils.compare_version(
                                model_info["f_fate_version"], "1.5.1"
                            )
                            == "lt"
                        ):
                            ...
                            model_info["initiator_role"] = (
                                model_info.get("f_train_runtime_conf", {})
                                .get("initiator", {})
                                .get("role")
                            )
                            model_info["initiator_party_id"] = (
                                model_info.get("f_train_runtime_conf", {})
                                .get("initiator", {})
                                .get("party_id")
                            )
                            ...
                            model_info["parent"] = (
                                False if model_info.get("f_inference_dsl") else True
                            )
            ...
            try:
                ...
                if model.exists():
                    ...
                    return send_file(
                        archive_file_path,
                        attachment_filename=os.path.basename(archive_file_path),
                        as_attachment=True,
                    )
                else:
        ...
        operation_record(
            data=job_runtime_conf, oper_type=model_operation, oper_status=""
        )
...
def tag_model(operation):
    ...
    if not model:
        raise Exception(
            "Can not found model by job id: '{}'.".format(request_data.get("job_id"))
        )
    ...
    if operation == "retrieve":
        ...
        tags = (
            Tag.select()
            .join(ModelTag, on=ModelTag.f_t_id == Tag.f_id)
            .where(ModelTag.f_m_id == model.f_model_version)
        )
        for tag in tags:
        ...
        if not tag:
            raise Exception(
                "Can not found '{}' tag.".format(request_data.get("tag_name"))
            )
        tags = (
            Tag.select()
            .join(ModelTag, on=ModelTag.f_t_id == Tag.f_id)
        ...
        if tag.f_name not in [t.f_name for t in tags]:
            raise Exception(
                "Model {} {} does not have tag '{}'.".format(
        ...
        return get_json_result(
            retmsg="'{}' tag has been removed from tag list of model {} {}.".format(
                request_data.get("tag_name"), model.f_model_id, model.f_model_version
            )
        )
    else:
        ...
        return get_json_result(
            retmsg="Adding {} tag for model with job id: {} successfully.".format(
                request_data.get("tag_name"), request_data.get("job_id")
            )
        )
...
def operate_tag(tag_operation):
    ...
    if tag_operation not in [
        TagOperation.CREATE,
        TagOperation.RETRIEVE,
        TagOperation.UPDATE,
        TagOperation.DESTROY,
        TagOperation.LIST,
    ]:
        raise Exception(
    ...
    if tag_operation == TagOperation.CREATE:
        try:
            if not tag_name:
                return get_json_result(
                    100,
                    "'{}' tag created failed. Please input a valid tag name.".format(
                        tag_name
                    ),
                )
            else:
        ...
        if not (
            tag_operation == TagOperation.RETRIEVE
        ...
        if tag_operation == TagOperation.RETRIEVE:
            if request_data.get("with_model", False):
                ...
                models = (
                    MLModel.select()
                    .join(ModelTag, on=ModelTag.f_m_id == MLModel.f_model_version)
                    .where(ModelTag.f_t_id == tag.f_id)
                )
                for model in models:
                    res["models"].append(
                        {
            ...
            if (tag.f_name == new_tag_name) and (tag.f_desc == new_tag_desc):
                ...
                if request_data.get("new_tag_name"):
                    if not Tag.get_or_none(Tag.f_name == new_tag_name):
                        ...
                        return get_json_result(
                            100, retmsg="'{}' tag already exists.".format(new_tag_name)
                        )

                tag.f_desc = new_tag_desc
                tag.save()
                return get_json_result(
                    retmsg="Infomation of '{}' tag has been updated successfully.".format(
                        tag_name
                    )
                )
            ...
            return get_json_result(
                retmsg="'{}' tag has been deleted successfully.".format(tag_name)
            )
    ...
    try:
        if oper_type == "migrate":
            ...
            OperLog.create(
                ...
                f_operation_status=oper_status,
                f_initiator_role=data.get("initiator").get("role"),
                f_initiator_party_id=data.get("party_id")
                if data.get("party_id")
                else data.get("initiator").get("party_id"),
                f_request_ip=request.remote_addr,
                f_model_id=data.get("model_id")
                if data.get("model_id")
                else data.get("job_parameters").get("model_id"),
                f_model_version=data.get("model_version")
                if data.get("model_version")
                else data.get("job_parameters").get("model_version"),
            )
        else:
            OperLog.create(
                f_operation_type=oper_type,
                f_operation_status=oper_status,
                f_initiator_role=data.get("role")
                if data.get("role")
                else data.get("initiator").get("role"),
                f_initiator_party_id=data.get("party_id")
                if data.get("party_id")
                else data.get("initiator").get("party_id"),
                f_request_ip=request.remote_addr,
...
def query_model():
    ...
    return Response(
        json.dumps(result, sort_keys=False, cls=DatetimeEncoder),
    ...
    if not model_info:
        ...
        for key, value in model_info.items():
            ...
            if version_check == "lt":
                ...
                model_init_role = (
                    value.get("f_initiator_role")
                    if value.get("f_initiator_role")
                    else value.get("f_train_runtime_conf", {})
                    .get("initiator", {})
                    .get("role", "")
                )
                model_init_party_id = (
                    value.get("f_initiator_role_party_id")
                    if value.get("f_initiator_role_party_id")
                    else value.get("f_train_runtime_conf", {})
                    .get("initiator", {})
                    .get("party_id", "")
                )
                if (init_role == model_init_role) and (
                    init_party_id == str(model_init_party_id)
                ):
                    break
        else:
            raise Exception(
        ...
        for role_name, role_partys in (
            value.get("f_train_runtime_conf", {}).get("role", {}).items()
        ):
        ...
        return get_json_result(
            retcode=(0 if deploy_status else 101),
...
def get_predict_dsl():
    ...
    return error_response(
        210, "No model found, please check if arguments are specified correctly."
    )
...
def get_predict_conf():
    ...
    if model_fp_list:
        fp = model_fp_list[0]
        pipeline_model = PipelinedModel(
            model_id=fp.split("/")[-2], model_version=fp.split("/")[-1]
        )
        pipeline = pipeline_model.read_component_model("pipeline", "pipeline")[
            "Pipeline"
        ]

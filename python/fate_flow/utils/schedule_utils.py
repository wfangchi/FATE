...
from fate_flow.scheduler.dsl_parser import DSLParser, DSLParserV2, BaseDSLParser
...
def get_job_dsl_parser_by_job_id(job_id):
    jobs = Job.select(
        Job.f_dsl, Job.f_runtime_conf_on_party, Job.f_train_runtime_conf
    ).where(Job.f_job_id == job_id)
    if jobs:
        job = jobs[0]
...
def get_job_dsl_parser(
    dsl=None, runtime_conf=None, pipeline_dsl=None, train_runtime_conf=None
):
    parser_version = str(runtime_conf.get("dsl_version", "1"))
    dsl_parser = get_dsl_parser_by_version(parser_version)
    setting_conf_path = os.path.join(
        file_utils.get_project_base_directory(), "conf", "ml_conf.yaml"
    )
...
def get_parser_version_mapping() -> Mapping[str, BaseDSLParser]:
    return {"1": DSLParser(), "2": DSLParserV2()}


def get_dsl_parser_by_version(version: str = "1") -> BaseDSLParser:
    mapping = get_parser_version_mapping()
    if isinstance(version, int):
        version = str(version)
    if version not in mapping:
        raise Exception(
            "{} version of dsl parser is not currently supported.".format(version)
        )
